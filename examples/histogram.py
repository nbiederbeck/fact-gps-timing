...
def plot_histogram():
    ...
    unix_time_utc = read_with_row_mask(
        table["UnixTimeUTC"], gps_triggers
    ).astype("uint64")
    timestamp = np.array(
        unix_time_utc[:, 0] * 1_000_000 + unix_time_utc[:, 1],
        dtype="datetime64[us]",
    )
    full_second = (timestamp + np.timedelta64(500, "ms")).astype(
        "datetime64[s]"
    )
